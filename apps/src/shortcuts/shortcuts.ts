  ...
  shortcutsRegistry.addHandlers({
    ...
    execute: async (node) => {
      ...
      navigator.clipboard
        ...
        .then(() => {
          Toast.success({
            content: MAPPING_TEXT.NODES_COPIED,
          });

        })
        .catch((err) => {
          Toast.error({
            content: MAPPING_TEXT.NODES_FAILED_COPIED,
          });
        });
    },
  });
  shortcutsRegistry.addHandlers({
    ...
    execute: async (selectedNodes?: WorkflowNodeEntity[]) => {
      ...
      if (clipboardData.fromHost !== window.location.host) {
        Toast.error({
          content: MAPPING_TEXT.NODES_PASTED_FAILED_FROM_DIFFERENT_PAGES,
        });
        return null;
      }
      ...
      const nodes = await Promise.all(
        clipboardData.nodes.map(async ({ nodeJSON }) => {
      ...
      if (nodes.length > 0) {
        selectService.selection = newnodes;
      }

      Toast.success({
        content: MAPPING_TEXT.NODES_PASTED,
      });
    },
  });
